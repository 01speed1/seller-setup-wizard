				...
				<div class="profile-info-summery">
					...
					<div class="profile-info">
						...
						<ul class="dokan-store-info">
							...
							<?php endif; ?>
							<?php 
							//We are talking about a virtual store, they dont have any addres to use directly
							/** 
							...
							<?php }
							*/  
							// end comment space need it to hide vendor address
							?>

							<?php  if ( isset( $store_info['store_desc'] ) && ! empty( $store_info['store_desc'] ) ) { ?>
								<li class="dokan-store-address dokan-store-description">
									<?php if ( $custom_layout ) : ?>
										<span class="label"><?php esc_html_e( 'Description:', 'martfury' ); ?></span>
									<?php else : ?>
										<i class="fa fa-map-marker"></i>
									<?php endif; ?>
									<?=esc_html($store_info['store_desc'])?>
								</li>
							<?php }  ?>

							<?php 
								//Daniela says 'vendor should not be contacted directly by phone'
							...
							<?php }
							*/ 
							// end space to hide vendor phone
							?>
							...
							<?php if ( $store_user->show_email() == 'yes' || $custom_layout ) { ?>
								<li class="dokan-store-email">
									...
									<?php endif; ?>
									<a href="mailto:<?php echo antispambot( $store_user->get_email() ); ?>"><strong><?php echo antispambot( $store_user->get_email() ); ?></strong></a>
								</li>
							<?php } ?>
							<?php
							//Daniela says that Rating should not appear yet 
							/*if ( ! $custom_layout ) : ?>
								<li class="dokan-store-rating">
									<i class="fa fa-star"></i>
									<?php dokan_get_readable_seller_rating( $store_user->get_id() ); ?>
