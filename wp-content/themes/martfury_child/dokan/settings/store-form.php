...
<?php
    ...
    * @param string $post_id  = ID of the post. When creating a new post, the parameter becomes NULL. When editing
    *                    a post, you get the edited post ID as the parameter
    * @param string $form_settings = An array of form settings
     */
    function queryAddInfoForm( $current_user, $profile_info ) {
        $profile_info['store_desc'] = get_user_meta( $current_user, 'dokan_store_description', true );
        $profile_info['gravatar'] = get_user_meta( $current_user, 'wp_user_avatar', true );
    }
    
    $storedesc      = isset( $profile_info['store_desc'] )? esc_attr( $profile_info['store_desc']) : '';

    $gravatar       = isset( $profile_info['wp-user-avatar'] ) ? absint( $profile_info['wp-user-avatar'] ) : 0;
...
?>
<?php 
    add_filter( 'dokan_settings_before_form', 'queryAddInfoForm', $current_user , $profile_info ); 
?>
    <form method="post" id="store-form"  action="" class="dokan-form-horizontal">
        ...
        <div class="dokan-form-group">
            <label class="dokan-w3 dokan-control-label" for="wp-user-avatar"><?php _e( 'Profile Picture', 'dokan-lite' ); ?></label>
            <!--Form Dokan - Imagen Vendor -->
            <div class="dokan-w5 dokan-gravatar">
                <div class="dokan-left gravatar-wrap<?php echo $gravatar ? '' : ' dokan-hide'; ?>">
                    ...
                    <input type="hidden" class="dokan-file-field" value="<?php echo $gravatar; ?>" name="wp-user-avatar">
        ...
        <!--Form Dokan - Nombre Vendor -->
        <div class="dokan-form-group">
            <label class="dokan-w3 dokan-control-label" for="dokan_store_name">
                <?php _e( 'Store Name', 'dokan-lite' ); ?>
            </label>

            <div class="dokan-w5 dokan-text-left">
                <input id="dokan_store_name" required value="<?=$storename; ?>" name="dokan_store_name" placeholder="<?php _e( 'store name', 'dokan-lite'); ?>" class="dokan-form-control" type="text">
            </div>
        </div>
        <!--Form Dokan - Descripción Tienda -->
        <div class="dokan-form-group">
            <label class="dokan-w3 dokan-control-label" for="dokan_store_description">
                <?php _e( 'Store Description', 'dokan-lite' ); ?>
            </label>

            <div class="dokan-w5 dokan-text-left">
                <textarea id="dokan_store_description" required value="<?=$storedesc?>" name="dokan_store_description" placeholder="<?php _e( 'Store Description', 'dokan-lite' ); ?>" class="dokan-form-control" type="text"><?=$storedesc;?></textarea>
            </div>
        </div>
        <?php /*
        <div class="dokan-form-group">
    ...
    <?php
        function save_extra_vendor_fields($store_id, $dokan_settings) {
            //echo '<script>console.log("hey acualiza!!")</script>';

            if ( isset( $_POST['dokan_store_description'] ) ) {
                $dokan_settings['dokan_store_description'] = $_POST['dokan_store_description'];
            }else{
                $dokan_settings['dokan_store_description']= 'Aquí su descripción de la tienda!';
            }
            if( isset( $_POST['wp-user-avatar'] ) ){
                $dokan_settings['dokan_gravatar'] = $_POST['wp-user-avatar'];
            }
            ...
            //update_user_meta( $store_id, 'dokan_store_description', $_POST['dokan_store_description'] );
        }
        

        //Before Render
        //do_action( 'dokan_store_profile_saved', $current_user, $profile_info );
    ?>
